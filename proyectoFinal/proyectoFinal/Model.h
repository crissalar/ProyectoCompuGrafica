...
private:
    ...
    {
        cout << "loadModel: leyendo archivo con Assimp..." << endl;
        scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        cout << "loadModel: ReadFile completado" << endl;
        ...
        cout << "Num materials: " << scene->mNumMaterials << endl;
        ...
        cout << "GlobalInverseTransform OK" << endl;

        processNode(scene->mRootNode, scene);
        cout << "processNode OK" << endl;
    }
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // normals (PROTECCIÓN: Validación contra mNormals NULL de Blender)
            ...
            // texture coordinates (PROTECCIÓN: Validación contra mTextureCoords NULL)
            ...
            // tangent (PROTECCIÓN: Validación contra mTangents NULL)
            ...
            // bitangent (PROTECCIÓN: Validación contra mBitangents NULL)
            ...
            // PROTECCIÓN: Validar estructura mBones antes de recorrerla
            if (mesh->mBones != nullptr) {
                for (unsigned int j = 0; j < mesh->mNumBones; j++) {
                    if (mesh->mBones[j] == nullptr) continue;
        ...
        // Process Bones (PROTECCIÓN: Validación mBones NULL)
        bones.clear();
        if (mesh->mBones != nullptr) {
            for (unsigned int i = 0; i < mesh->mNumBones; i++) {
                if (mesh->mBones[i] == nullptr) continue;
        ...
        // (PROTECCIÓN: Descartar caras rotas, degeneradas o que no tengan exactamente 3 índices)
    ...
    vector<Texture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                textures_loaded.push_back(texture);  // store it as loaded optimization
            }
        }
        return textures;
    }
